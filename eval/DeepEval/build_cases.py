...
def build_conversational_case(story: Dict[str, Any]) -> ConversationalTestCase:
    ...
    turns = [
        Turn(role="user", content=f"User story {story_id}\n\n{payloads['agent1_input']}"),
        Turn(role="assistant", content=payloads["agent1"]),
        Turn(role="user", content="Refine the business model and workflow from Agent 1."),
        ...
        Turn(role="user", content="Validate the generated tests and highlight gaps or ambiguities."),
...
def build_conversational_cases(input_path: str | Path, max_cases: int | None = None) -> List[ConversationalTestCase]:
...
def export_case_summary(cases: List[ConversationalTestCase], output_path: str | Path) -> None:
    payload = []
    for case in cases:
        payload.append(
            {
