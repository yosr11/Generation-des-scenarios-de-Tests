...
def load_existing_results(output_json_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"  [WARN] Fichier {output_json_path} illisible ou corrompu, on repart de zéro.")
        return []
...
def save_progress(results: List[Dict[str, Any]], output_json_path: Path, input_json_path: str) -> None:
...
def run_geval_evaluation(
    ...
    if not force_restart:
        ...
        if already_done_ids:
            print(f"[REPRISE] {len(already_done_ids)} story(ies) déjà évaluée(s), on les saute : "
                  f"{sorted(already_done_ids)}")

    stories_to_run = [s for s in stories if s.get("story_id", "unknown") not in already_done_ids]
    ...
    if not stories_to_run:
        ...
        return {"meta": {"input_file": str(input_json_path), "case_count": len(results)},
    ...
    for story_idx, story in enumerate(stories_to_run):
        ...
        agent1_facts_str = serialize_payload({
            ...
            "acceptance_criteria_explicit": agent1_output.get("acceptance_criteria_explicit", []),
            "testable_points": agent1_output.get("testable_points", []),
        })
        ...
        agent3_str = serialize_payload({
            ...
            "uncovered_testable_points": agent3_output.get("uncovered_testable_points", []),
        ...
        try:
            ...
            print("[INFO] Progression sauvegardée. Relance le script plus tard pour reprendre.")
        ...
        save_progress(results, output_json_path, str(input_json_path))
        print(f"  [OK] Story {story_id} sauvegardée ({len(results)} story(ies) au total).")

        if story_idx < len(stories_to_run) - 1:
            print(f"  ... pause de {PAUSE_BETWEEN_STORIES_SEC}s avant la story suivante ...")
...
def build_geval_summary(results: List[Dict[str, Any]], metric_names: List[str]) -> Dict[str, Any]:
    ...
    for name in metric_names:
        ...
        summary[name] = {
            ...
            "taux_sous_seuil": round(sum(1 for s in scores if s < 0.7) / len(scores), 2) if scores else None,
            "pires_stories": worst,
        }
    return summary


def print_geval_summary(results: List[Dict[str, Any]], summary: Dict[str, Any], metric_names: List[str]) -> None:
    print(f"\n{'='*70}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Le script s'est arrêté avant la fin. Relance-le pour reprendre là où il s'est arrêté.")