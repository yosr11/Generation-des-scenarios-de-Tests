...
def upgrade() -> None:
    ...
    with op.batch_alter_table("users") as batch_op:
        ...
        batch_op.add_column(
            sa.Column("jira_username", sa.String(255), nullable=True, index=True)
        )
        batch_op.add_column(
            sa.Column(
                "is_active",
    ...
    try:
        op.create_index(
            "ix_users_jira_username", "users", ["jira_username"], unique=False
        )
    except Exception:
    ...
    op.create_table(
        ...
        sa.Column(
            "use_rag", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
            "use_legacy_rag",
            sa.Boolean(),
        ...
        sa.Column(
            "run_agent4", sa.Boolean(), nullable=False, server_default=sa.text("true")
        ),
        sa.Column(
            "started_at",
    ...
    op.create_index(
        "ix_pipeline_runs_story_id", "pipeline_runs", ["story_id"], unique=False
    )
    op.create_index(
        "ix_pipeline_runs_launched_by", "pipeline_runs", ["launched_by"], unique=False
    )
