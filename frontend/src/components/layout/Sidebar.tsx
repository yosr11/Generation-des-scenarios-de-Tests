...
import {
  GitBranch, History, FolderKanban, LogOut, ChevronRight,
...
const TESTER_NAV = [
  ...
  { to: '/history',   label: 'Historique',  icon: History,      color: 'text-brand-orange' },
  { to: '/projects',  label: 'Projets',     icon: FolderKanban, color: 'text-brand-violetlt', testerOnly: true },
]
...
export const Sidebar: React.FC = () => {
  ...
  const navItems = isAdmin ? ADMIN_NAV : TESTER_NAV.filter(n => !n.testerOnly || user?.role === 'tester')
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(244,63,94,0.12) 0%, transparent 70%)', filter: 'blur(30px)' }} />
      <div className="absolute top-1/2 right-0 w-32 h-32 rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.1) 0%, transparent 70%)', filter: 'blur(20px)' }} />
      ...
      <div className="relative z-10 mx-3 mt-4">
        {isAdmin ? (
          <div className="px-3 py-2.5 rounded-xl flex items-center gap-2"
            ...
            <ShieldCheck size={14} className="text-brand-rose flex-shrink-0" />
      ...
      <nav className="relative z-10 flex-1 px-3 py-4 space-y-1">
        ...
        {navItems.map(({ to, label, icon: Icon, color }) => (
          <NavLink key={to} to={to} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            {({ isActive }) => (
              <>
                <Icon size={17} className={isActive ? 'text-brand-rose' : `${color} opacity-70`} />
      ...
      <div className="relative z-10 px-3 pb-4 border-t border-white/08 pt-4">
        <div className="flex items-center gap-3 px-2 mb-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
            style={{ background: isAdmin ? 'linear-gradient(135deg,#ef4444,#f43f5e)' : 'var(--grad-violet)' }}>
            {initials}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <span className="inline-block text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full mt-0.5"
              style={{
                background: isAdmin ? 'rgba(244,63,94,0.2)' : 'rgba(124,58,237,0.2)',
