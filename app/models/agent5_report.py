...
class ReportStory(BaseModel):
    ...
    story_type: str = Field(..., description="Type (functional, technical, poc_or_study, etc.)")
    ...
    technical_scope: List[str] = Field(default_factory=list, description="Périmètre technique (si applicable)")
...
class ReportTestSuite(BaseModel):
    ...
    tests_summary: List[ReportTestCase] = Field(default_factory=list, description="Liste simplifiée des tests")


class ReportCoverageMetrics(BaseModel):
    """Section : Indicateurs de Couverture."""
    coverage_rate: float = Field(..., ge=0, le=100, description="Taux de couverture (%)")
    total_testable_points: int = Field(..., description="Nombre total de points testables")
    covered_points: int = Field(..., description="Nombre de points couverts")
    uncovered_points: List[str] = Field(default_factory=list, description="Points non couverts")
    coverage_status: str = Field(..., description="EXCELLENT (>85%), GOOD (70-85%), FAIR (50-70%), POOR (<50%)")
...
class ReportQualityIssue(BaseModel):
    ...
    affected_tests: List[str] = Field(default_factory=list, description="Noms de tests affectés")
...
class ReportQualityAssurance(BaseModel):
    ...
    duplicate_pairs: int = Field(..., description="Nombre de paires dupliquées détectées")
    ambiguity_count: int = Field(..., description="Nombre d'étapes ambiguës détectées")
    issues: List[ReportQualityIssue] = Field(default_factory=list, description="Détail des issues")
    llm_quality_score: Optional[int] = Field(default=None, ge=0, le=10, description="Score qualitatif global (0-10)")
    llm_quality_summary: Optional[str] = Field(default="", description="Feedback qualitatif du LLM")
...
class ReportExecutiveSummary(BaseModel):
    ...
    overall_status: str = Field(..., description="APPROVED, REQUIRES_REVIEW, NEEDS_REWORK")
    key_findings: List[str] = Field(default_factory=list, description="Findings principaux (max 5)")
    next_steps: List[str] = Field(default_factory=list, description="Prochaines étapes recommandées")
...
class Agent5Report(BaseModel):
    ...
    generated_timestamp: str = Field(..., description="ISO 8601 timestamp de génération")
    
    # Sections principales
    executive_summary: ReportExecutiveSummary = Field(..., description="Résumé exécutif")
    ...
    coverage_metrics: ReportCoverageMetrics = Field(..., description="Métriques de couverture")
    quality_assurance: ReportQualityAssurance = Field(..., description="Résultats validation/QA")
    ...
    processing_notes: List[str] = Field(
        default_factory=list,
        description="Notes techniques de traitement"
    )


class Agent5ReportRequest(BaseModel):
    """Requête pour génération rapport (body optionnel du POST)."""
    output_format: str = Field(default="json", description="Format output: json, markdown")
...
class Agent5ReportResponse(BaseModel):
    ...
    report: Optional[Agent5Report] = Field(default=None, description="Rapport structuré")
    report_markdown: Optional[str] = Field(default=None, description="Version markdown (si demandée)")
