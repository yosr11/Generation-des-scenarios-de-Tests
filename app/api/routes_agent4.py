"""
Agent 4 — validateur pur (FastAPI).

Charge automatiquement depuis la base SQLite :
- testable_points (dernière analyse Agent 1) ;
- tests manuels (dernier résultat Agent 3 enregistré).

Aucune génération, aucun appel Agent 3 ici (sauf route /correct qui lance la boucle corrective).
...
from app.models.agent4_validation import Agent4StoryDashboard, Agent4ValidationResult
...
router = APIRouter(prefix="/agent4", tags=["Agent 4 — Test Validator"])
logger = logging.getLogger(__name__)


class Agent4ThresholdsBody(BaseModel):
...
class Agent4ValidateFromDbRequest(Agent4ThresholdsBody):
    """Validation pour une story identifiée dans le corps."""

    story_id: str = Field(
        ..., min_length=1, description="Identifiant story (même id qu'en base)"
    )


def _load_testable_points_and_tests(
    story_id: str,
) -> Tuple[List[str], List[ManualTestCase], str]:
    ...
    if not raw_tests:
        raise HTTPException(
            status_code=404,
            detail=(
                f"Aucun test Agent 3 enregistré pour {story_id}. "
                f"Lancez d'abord la génération Agent 3, puis réessayez."
            ),
        )
...
@router.post(
    ...
    response_model=Agent4StoryDashboard,
    summary="Tableau de bord validation (données Agent 1 + 2 depuis la base)",
)
def post_agent4_story_dashboard(story_id: str, body: Agent4ThresholdsBody):
...
@router.post(
    ...
    response_model=Agent4ValidationResult,
    summary="Valider les tests (Agent 4) — story_id dans le corps",
    response_description="Tests lus depuis la base + rapport de validation",
)
def post_validate_and_improve(body: Agent4ValidateFromDbRequest):
...
@router.post(
    ...
    response_model=Agent4ValidationResult,
    summary="Valider les tests (Agent 4) — story_id dans l'URL",
)
def post_validate_and_improve_by_story(story_id: str, body: Agent4ThresholdsBody):
    points, tests, story_summary = _load_testable_points_and_tests(story_id)
...
class Agent4CorrectBody(Agent4ThresholdsBody):
    """Paramètres pour la boucle corrective (validation + gap-fill Agent 3)."""

    max_iterations: int = Field(
        default=2, ge=1, le=5, description="Nombre max de tentatives de correction"
    )
    gap_fill_model_alias: str = Field(
        default="nova-lite-2",
        description="Modèle LLM pour la génération gap-fill Agent 3",
    )
...
@router.post(
    "/correct/{story_id}",
    response_model=Agent4ValidationResult,
    summary="Validation + boucle corrective (rappel Agent 3 si couverture insuffisante)",
)
def post_validate_and_correct(story_id: str, body: Agent4CorrectBody):
    """
    1. Valide les tests existants (comme validate-and-improve)
    2. Si couverture < seuil : appelle Agent 3 pour générer des tests sur les points manquants
    ...
    if not analysis:
        raise HTTPException(
            status_code=404, detail=f"Aucune analyse Agent 1 pour {story_id}"
        )
    ...
    if not raw_tests:
        raise HTTPException(
            status_code=404, detail=f"Aucun test Agent 3 pour {story_id}"
        )
    ...
    for iteration in range(1, body.max_iterations + 1):
        logger.info(
            f"[Agent4-Correct] Iteration {iteration}/{body.max_iterations} for {story_id}"
        )
        ...
        if (
            best_result is None
            or result.report.coverage_rate > best_result.report.coverage_rate
        ):
            best_result = result
        ...
        if result.report.coverage_rate >= body.coverage_threshold:
            logger.info(
                f"[Agent4-Correct] Coverage OK ({result.report.coverage_rate:.1%}) at iteration {iteration}"
            )
            break
        ...
        # Call Agent 3 gap-fill
        logger.info(
            f"[Agent4-Correct] Coverage {result.report.coverage_rate:.1%} < {body.coverage_threshold:.1%}, "
            f"calling Agent 3 for {len(uncovered)} uncovered points"
        )
        try:
            ...
            if gap_result.tests:
                ...
                logger.info(
                    f"[Agent4-Correct] +{len(gap_result.tests)} tests from gap-fill"
                )
            else:
                logger.warning("[Agent4-Correct] Gap-fill returned 0 tests, stopping")
                break

        except Exception as e:
            logger.error(f"[Agent4-Correct] Gap-fill failed: {e}")
            break
    ...
    save_manual_tests_snapshot(
        story_id, final_tests_dicts, generation_model=body.gap_fill_model_alias
    )
