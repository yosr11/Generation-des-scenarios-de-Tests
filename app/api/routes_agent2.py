"""
app/api/routes_agent2.py
──────────────────────────
Endpoints REST pour Agent 2 — QA Business Modeling.

Endpoints :
  GET  /agent2/{story_id}          — Lancer la modélisation (depuis l'analyse existante en DB)
  POST /agent2/{story_id}/run      — Forcer une nouvelle modélisation (ignore le cache)
  GET  /agent2/{story_id}/latest   — Lire le dernier business model en base
"""
...
router = APIRouter(prefix="/agent2", tags=["Agent 2 — Business Modeling"])
...
def run_business_modeling(
    ...
    """
    Lance Agent 2 pour une story déjà analysée par Agent 1.
    ...
    if not force:
        ...
        if cached:
            logger.info(
                f"[Agent 2 API] Cache hit pour {story_id} (modèle={model_alias})"
            )
    ...
    if story_type != "functional":
        raise HTTPException(
            ...
            detail=(
                ...
                "Agent 2 ne traite que les stories fonctionnelles."
            ),
        )

    # Lancer la modélisation
    logger.info(
        f"[Agent 2 API] Modélisation de {story_id} (modèle={model_alias}, force={force})"
    )
...
def get_latest(
    ...
    if not cached:
        raise HTTPException(
            status_code=404,
            detail=f"Aucun business model trouvé pour '{story_id}'. Lancez d'abord GET /agent2/{story_id}.",
        )
    return _build_response(story_id, cached)


# ─── GET /agent2/{story_id}/history ─────────────────────────────────────────
