...
from app.services.story_analysis_service import (
    ...
    StoryAnalysisError,
    _is_reference_only_description,
)
...
def preview_legacy_rag_examples(
    ...
    min_score: float = Query(
        0.55, ge=0.0, le=1.0, description="Score minimum (similarité cosinus)"
    ),
    min_steps: int = Query(
        2, ge=0, description="Nombre minimum d'étapes du test legacy"
    ),
    min_action_chars: int = Query(
        12, ge=0, description="Longueur minimum d'au moins une action"
    ),
):
    """
...
def generate_manual_tests_for_story_data(
    ...
    if not description:
        return ManualTestGenerationResult(
            ...
            tests=[],
            notes=[
                "Action requise : demander au PO d’expliquer le besoin dans la description de la story avant de relancer la génération."
            ],
        )

    story_type = (analysis.get("story_type") or "").strip().lower()
    if (
        _is_reference_only_description(description)
        and story_type == "invalid_or_too_weak"
    ):
        return ManualTestGenerationResult(
            story_id=story_id,
            recommended_test_strategy="needs_refinement",
    ...
    if story_type == "invalid_or_too_weak":
        return ManualTestGenerationResult(
            ...
            tests=[],
            notes=[
                "Action requise : la story doit être complétée ou reformulée par le PO."
            ],
        )
...
def generate_manual_tests(
    ...
    model_alias: str = Query(
        "nova-lite-2", description=f"Model alias. Allowed: {list(ALL_MODELS.keys())}"
    ),
    ...
    if model_alias not in ALLOWED_MODELS:
        raise HTTPException(
            status_code=400,
            detail=f"Model alias invalide '{model_alias}'. Allowed: {ALLOWED_MODELS}",
        )
    ...
    if not story:
        raise HTTPException(
            status_code=404,
            detail=f"Story introuvable en base : {story_id}. Lancez d'abord l'analyse via /analysis/{story_id}.",
        )
    ...
    if use_legacy_rag:
        try:
            ...
            if legacy_examples:
                ...
                logger.info(
                    f"[/manual-tests/generate/{story_id}] Legacy RAG: no example >= min_score"
                )
        except Exception as e:
            logger.warning(
                f"[/manual-tests/generate/{story_id}] Legacy RAG failed: {e}"
            )
            legacy_examples = None
...
def generate_manual_tests_for_epic(
    epic_key: str,
    analysis_model: str = Query(
        "nova-lite-2",
        description=f"Modèle pour l'Agent 1 (analyse). Allowed: {list(GROQ_MODELS.keys())}",
    ),
    ...
    if analysis_model not in ALLOWED_MODELS:
        raise HTTPException(
            status_code=400,
            detail=f"Model alias invalide '{analysis_model}'. Allowed: {ALLOWED_MODELS}",
        )
    if generation_model not in ALL_MODELS:
        raise HTTPException(
            status_code=400,
            detail=f"Model alias invalide '{generation_model}'. Allowed: {list(ALL_MODELS.keys())}",
        )
    ...
    service = ManualTestGeneratorService(
        llm_client=llm_client, model_name=generation_model_name
    )
    ...
    for raw in raw_stories:
        ...
        try:
            ...
            if _is_reference_only_description(description):
                logger.info(
                    f"Story {story_id} : description = référence seule, résolution via Agent 1"
                )
            ...
            if existing_analysis:
                logger.info(
                    f"Story {story_id} : analyse existante trouvée en base, skip Agent 1"
                )
                analysis_dict = existing_analysis
            else:
                try:
                    analysis = analyze_story_with_groq(
                        story=enriched, model_alias=analysis_model
                    )
                    ...
                    errors.append(
                        {"story_id": story_id, "step": "analysis", "error": str(e)}
                    )
                    continue
            ...
            if story_type == "invalid_or_too_weak":
                skipped.append(
                    {"story_id": story_id, "reason": "Story invalide ou trop faible"}
                )
                continue
            ...
            if use_legacy_rag:
                try:
                    ...
                    logger.warning(
                        f"[epic={epic_key}] Story {story_id}: Legacy RAG failed: {e}"
                    )
                    legacy_examples = None
