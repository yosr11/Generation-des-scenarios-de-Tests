...
class Agent3CorrectBody(Agent3ThresholdsBody):
    ...
    gap_fill_model_alias: str = Field(
        default="nova-lite-2",
        description="Modèle LLM pour la génération gap-fill Agent 2",
    )
...
def post_validate_and_correct(story_id: str, body: Agent3CorrectBody):
    ...
    if not raw_tests:
        raise HTTPException(
            status_code=404, detail=f"Aucun test Agent 2 pour {story_id}"
        )
    ...
    for iteration in range(1, body.max_iterations + 1):
        logger.info(
            f"[Agent3-Correct] Iteration {iteration}/{body.max_iterations} for {story_id}"
        )
        ...
        try:
            ...
            if gap_result.tests:
                ...
                logger.info(
                    f"[Agent3-Correct] +{len(gap_result.tests)} tests from gap-fill"
                )
            else:
